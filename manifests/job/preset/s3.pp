...
)
{
  ...
  $_fileset = regsubst($title, "(.+/)?${client_name}-(.*)${bareos::client::job_suffix}(-base)?$", '\2')

  if $base == '' {
    # If we are using a base job, let the base job declare the fileset in use
    ...
    ensure_resource('bareos::job::preset::s3::config',
                    $_fileset,
